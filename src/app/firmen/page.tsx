...
export default async function FirmenPage({ searchParams }: Props) {
  ...
  const [data, stats] = await Promise.all([
    ...
    fetchApi<CompanyStatsResponse>('/public/companies/stats?portal_id=3'),
  ]);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-amber-50/50 to-white">
      ...
      <section className="relative pt-24 pb-12 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-amber-600 via-orange-600 to-orange-500" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-lg text-white/80 max-w-2xl mx-auto mb-8">
            Finde deinen Traum-Arbeitgeber. Vergleiche Arbeitgeber, lies Bewertungen und entdecke offene Stellen.
          </p>
          ...
          <form className="max-w-2xl mx-auto" action="/firmen">
            <div className="flex bg-white rounded-xl shadow-xl overflow-hidden">
              ...
              <button
                type="submit"
                className="px-6 bg-amber-600 hover:bg-amber-700 text-white font-semibold transition-colors"
              >
                Suchen
              </button>
            </div>
          ...
          <aside className="hidden lg:block">
            ...
            {stats && (
              <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
                ...
                <div className="text-3xl font-extrabold text-amber-600 mb-1">{stats.totalCompanies}</div>
                <div className="text-sm text-gray-500 mb-4">Arbeitgebere</div>
              </div>
            )}
                  ...
                  {stats.topCities.slice(0, 8).map((city) => (
                    <li key={city.name}>
                      <Link
                        ...
                        className="flex items-center justify-between text-sm hover:text-amber-600 transition-colors"
                      >
          ...
          <div className="lg:col-span-3">
              ...
              <div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                {searchParams.q && (
                  <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 px-2.5 py-1 rounded-full text-xs font-medium">
                    &quot;{searchParams.q}&quot;
                    <Link href="/firmen" className="hover:text-violet-900">×</Link>
                  </span>
                )}
                {searchParams.city && (
                  <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 px-2.5 py-1 rounded-full text-xs font-medium">
                    {searchParams.city}
                    <Link href={`/firmen${searchParams.q ? `?q=${searchParams.q}` : ''}`} className="hover:text-violet-900">×</Link>
                  </span>
                )}
              </div>
            ...
            {companies.length > 0 ? (
              <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {companies.map((company) => (
                  <Link
                    ...
                    className="group bg-white rounded-xl border border-gray-200 hover:border-amber-300 hover:shadow-lg transition-all p-5"
                  >
                    ...
                    {company.industry && (
                      <span className="inline-block text-xs font-medium bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full mb-2">
                        {company.industry}
                      </span>
                    )}
              ...
              <div className="text-center py-16 bg-white rounded-xl border border-gray-200">
                ...
                <Link href="/firmen" className="text-amber-600 hover:text-amber-700 font-medium">
      ...
      <section className="bg-gradient-to-r from-amber-600 to-orange-500 py-12">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-white/80 mb-6">Erstelle dein Firmenprofil und finde die besten Bewerber über unsere Plattform.</p>
          <a
