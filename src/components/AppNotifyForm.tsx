...
export default function AppNotifyForm({
    ...
    return (
      <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl p-6 sm:p-8">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-semibold text-amber-700 uppercase tracking-wide">Bald verfügbar</span>
        </div>
        ...
        {status === 'success' ? (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2">
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-xl border border-amber-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
            />
            <button
              type="submit"
              disabled={status === 'loading'}
              className="px-5 py-2.5 bg-gradient-to-r from-amber-600 to-orange-600 text-white font-semibold rounded-xl text-sm hover:from-amber-700 hover:to-orange-700 transition-all disabled:opacity-60 flex items-center justify-center gap-2"
            >
        ...
        {status === 'success' ? (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input
              ...
              onChange={(e) => setEmail(e.target.value)}
              placeholder="Deine E-Mail-Adresse"
              className="flex-1 px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
            />
            <button
              type="submit"
              disabled={status === 'loading'}
              className="px-6 py-3 bg-gradient-to-r from-amber-600 to-orange-600 text-white font-semibold rounded-xl hover:from-amber-700 hover:to-orange-700 transition-all disabled:opacity-60 flex items-center justify-center gap-2"
            >
              {status === 'loading' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Bell className="h-4 w-4" />}
              Erinnere mich
            </button>
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-r from-amber-500 to-orange-600 p-8 sm:p-10 text-center text-white">
      <Smartphone className="h-10 w-10 mx-auto mb-4 opacity-90" />
      <h2 className="text-xl sm:text-2xl font-bold mb-2">{title}</h2>
      <p className="text-amber-100 mb-6 max-w-lg mx-auto">{description}</p>
      {status === 'success' ? (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
          ...
          <button
            type="submit"
            disabled={status === 'loading'}
            className="px-6 py-3 bg-white text-amber-700 font-semibold rounded-xl hover:bg-amber-50 transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
          >
            {status === 'loading' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Bell className="h-4 w-4" />}
            Erinnere mich
          </button>
