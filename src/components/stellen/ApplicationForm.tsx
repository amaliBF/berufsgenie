...
export default function ApplicationForm({ jobId, jobTitle, companyName }: { jobId: string; jobTitle: string; companyName: string }) {
  ...
  const inputClass = "mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none";
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        {isActive('phone') && (
          <div>
            ...
            <input name="phone" type="tel" required={isRequired('phone')} className={inputClass} />
          </div>
        )}
        ...
        {isActive('birthDate') && (
          <div>
            ...
            <input name="birthDate" type="date" required={isRequired('birthDate')} className={inputClass} />
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-3">
            {isActive('postalCode') && (
              <div>
                ...
                <input name="postalCode" required={isRequired('postalCode')} className={inputClass} />
              </div>
            )}
            {isActive('city') && (
              <div>
                <label className="text-xs font-medium text-gray-700">Stadt{isRequired('city') ? ' *' : ''}</label>
                <input name="city" required={isRequired('city')} className={inputClass} />
              </div>
            )}
          </div>
        )}
        ...
        {isActive('schulabschluss') && (
          <div>
            ...
            <select name="schulabschluss" required={isRequired('schulabschluss')} className={`${inputClass} bg-white`}>
        ...
        {isActive('message') && (
          <div>
            ...
            <textarea name="message" rows={3} required={isRequired('message')} className={`${inputClass} resize-none`} placeholder="Warum interessiert dich diese Stelle?" />
          </div>
        )}
        ...
        {isActive('documents') && (
          <div>
            ...
            <input
              ...
              className="mt-1 w-full text-sm file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100"
            />
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white font-medium py-2.5 px-4 rounded-xl text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {submitting ? (
            <>
